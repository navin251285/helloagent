#!/usr/bin/env python3
"""
MCP Client for Patient Management with Chroma DB Search and Ollama Summary Generation
WebSocket Version - Connects to MCP server via WebSocket instead of stdio
Workflow:
1. User enters disease/symptom keyword
2. Chroma DB searches and returns top 5 matching patients
3. User selects one patient
4. Ollama generates smart summary from patient health data
5. Summary saved to patient_summaries.csv via MCP server
"""

import asyncio
import json
import websockets


class MCPWebSocketClient:
    """MCP client that communicates via WebSocket"""
    
    def __init__(self, uri="ws://127.0.0.1:8765/mcp"):
        self.uri = uri
        self.websocket = None
        self.request_id = 0
    
    async def connect(self):
        """Connect to the WebSocket server"""
        print(f"\n[WebSocket] Connecting to {self.uri}...", flush=True)
        # Increase timeouts for long-running LLM operations
        self.websocket = await websockets.connect(
            self.uri,
            ping_timeout=120,  # 2 minutes for LLM inference
            close_timeout=10
        )
        print(f"[WebSocket] ✅ Connected successfully", flush=True)
        print(f"[WebSocket] Configuration: ping_timeout=120s, close_timeout=10s", flush=True)
        
        # Initialize the MCP session
        await self.initialize()
    
    async def close(self):
        """Close the WebSocket connection"""
        if self.websocket:
            print(f"\n[WebSocket] Closing connection...", flush=True)
            await self.websocket.close()
            print(f"[WebSocket] Connection closed", flush=True)
    
    async def send_request(self, method, params=None, handle_streaming=False):
        """Send a JSON-RPC request and wait for response
        
        Args:
            method: RPC method name
            params: RPC parameters
            handle_streaming: If True, watch for stream_token messages during generation
        """
        self.request_id += 1
        request = {
            "jsonrpc": "2.0",
            "id": self.request_id,
            "method": method,
            "params": params or {}
        }
        
        # Show outgoing request
        print(f"\n[WebSocket →] Sending: {method}", flush=True)
        if params:
            param_summary = str(params)[:100]
            print(f"[WebSocket →] Params: {param_summary}{'...' if len(str(params)) > 100 else ''}", flush=True)
        
        await self.websocket.send(json.dumps(request))
        
        # Wait for response, handling streaming tokens if needed
        print(f"[WebSocket ←] Waiting for response...", flush=True)
        
        full_response = None
        streaming_active = False
        
        while True:
            message_text = await self.websocket.recv()
            message = json.loads(message_text)
            
            # Handle streaming tokens (special messages from server)
            if message.get("type") == "stream_token":
                if not streaming_active:
                    print(f"\n[🔄 Streaming] Starting token stream: ", end="", flush=True)
                    streaming_active = True
                token = message.get("token", "")
                print(token, end="", flush=True)
                continue
            
            # This is the actual response
            full_response = message
            
            if streaming_active:
                print(f"\n[🔄 Streaming] Tokens complete", flush=True)
            
            # Show response summary
            if "result" in full_response:
                print(f"[WebSocket ←] Response received: {method} → Success", flush=True)
            elif "error" in full_response:
                print(f"[WebSocket ←] Response received: {method} → Error", flush=True)
                raise Exception(f"Server error: {full_response['error']['message']}")
            
            break
        
        return full_response.get("result")
    
    async def initialize(self):
        """Initialize the MCP session"""
        print(f"\n[MCP Protocol] Initializing session...", flush=True)
        result = await self.send_request("initialize", {
            "protocolVersion": "2024-11-05",
            "capabilities": {},
            "clientInfo": {
                "name": "patient-client",
                "version": "1.0.0"
            }
        })
        server_name = result['serverInfo']['name']
        server_version = result.get('serverInfo', {}).get('version', 'unknown')
        print(f"[MCP Protocol] ✅ Session initialized", flush=True)
        print(f"[Server Info] Name: {server_name}, Version: {server_version}", flush=True)
        return result
    
    async def list_tools(self):
        """List available tools from the server"""
        result = await self.send_request("tools/list")
        return result.get("tools", [])
    
    async def call_tool(self, tool_name, arguments):
        """Call a specific tool on the server
        
        For generate_summary tool, streaming is enabled to show tokens in real-time.
        """
        # Enable streaming for generate_summary tool
        handle_streaming = (tool_name == "generate_summary")
        
        result = await self.send_request("tools/call", {
            "name": tool_name,
            "arguments": arguments
        }, handle_streaming=handle_streaming)
        return result.get("content", [])


async def search_patients_by_disease(client, disease_keyword):
    """Call MCP server to search patients by disease"""
    print(f"\n[Tool Call] search_patients_by_disease(disease_keyword='{disease_keyword}')", flush=True)
    result = await client.call_tool("search_patients_by_disease", {
        "disease_keyword": disease_keyword
    })
    print(f"[Tool Result] Received {len(result[0]['text'])} characters of search results", flush=True)
    return result[0]["text"] if result else ""


async def get_patient_summary(client, patient_id):
    """Call MCP server to get patient summary"""
    print(f"\n[Tool Call] get_patient_summary(patient_id='{patient_id}')", flush=True)
    result = await client.call_tool("get_patient_summary", {
        "patient_id": str(patient_id)
    })
    print(f"[Tool Result] Received patient details", flush=True)
    return result[0]["text"] if result else ""


async def generate_summary(client, patient_id):
    """Call MCP server to generate summary using Ollama with streaming tokens"""
    print(f"[Tool Call] generate_summary(patient_id='{patient_id}')", flush=True)
    print(f"[Ollama] ⏳ Requesting AI summary generation WITH STREAMING TOKENS...", flush=True)
    print(f"[Progress] 📡 Connecting to Ollama model...", flush=True)
    print(f"[Streaming] 🔄 Starting token stream - tokens will appear below as they're generated:\n", flush=True)
    
    result = await client.call_tool("generate_summary", {
        "patient_id": str(patient_id)
    })
    
    print(f"\n[Tool Result] ✅ Stream complete - Received full summary ({len(result[0]['text'])} chars)", flush=True)
    return result[0]["text"] if result else ""


async def update_summary(client, patient_id, summary):
    """Call MCP server to update and save summary"""
    print(f"\n[Tool Call] update_patient_summary(patient_id='{patient_id}')", flush=True)
    print(f"[CLIENT] Calling update_summary with:")
    print(f"[CLIENT]   Patient ID: {patient_id}")
    print(f"[CLIENT]   Summary length: {len(summary)} chars")
    print(f"[CLIENT]   Summary first 100 chars: {summary[:100]}")
    
    result = await client.call_tool("update_patient_summary", {
        "patient_id": str(patient_id),
        "summary": summary
    })
    
    output = result[0]["text"] if result else ""
    print(f"\n[Tool Result] Update completed")
    print(f"[CLIENT] Server response:")
    print(f"{output}")
    return output


async def main():
    """Main client function"""
    
    print("\n" + "="*100)
    print("🏥 PATIENT SUMMARY GENERATION SYSTEM (WebSocket Version)")
    print("="*100)
    print("\nWorkflow: Disease Search → Select Patient → Generate Summary (via Ollama Phi)")
    print("="*100 + "\n")
    
    # Create and connect WebSocket client
    client = MCPWebSocketClient()
    
    try:
        await client.connect()
        
        while True:
            # Step 1: Get disease keyword from user
            print("\n" + "-"*100)
            print("STEP 1: SEARCH PATIENTS BY DISEASE/SYMPTOMS")
            print("-"*100)
            disease_keyword = input("\nEnter disease/symptom keyword (e.g., 'diabetes', 'chest pain', 'hypertension'): ").strip()
            
            if not disease_keyword.lower() or disease_keyword.lower() == 'quit':
                print("\n✓ Exiting system. Goodbye!")
                break
            
            # Step 2: Search for patients
            print(f"\n🔍 Searching for patients with '{disease_keyword}'...")
            print(f"[Progress] Querying Chroma DB for semantic matches...", flush=True)
            search_results = await search_patients_by_disease(client, disease_keyword)
            
            print("\n" + "="*100)
            print(f"SEARCH RESULTS FOR: {disease_keyword.upper()}")
            print("="*100)
            print(search_results)
            
            # Parse results to extract patient IDs
            lines = search_results.split('\n')
            patients = []
            for line in lines:
                if line.strip().startswith(('1.', '2.', '3.', '4.', '5.')):
                    # Extract patient ID from the line
                    # Format: "1. ID: 11  | Name: Jerry Rivera | Age: 59 | Symptoms: ..."
                    parts = line.split('|')
                    if len(parts) >= 1:
                        # Extract ID from "1. ID: 11  "
                        id_part = parts[0].split(':')[1].strip()
                        patients.append(id_part)
            
            if not patients:
                print("\nNo patients found. Please try a different search term.")
                continue
            
            # Step 3: Get user to select a patient
            print("\n" + "-"*100)
            print("STEP 2: SELECT A PATIENT")
            print("-"*100)
            while True:
                try:
                    choice = input(f"\nSelect patient number (1-{len(patients)}) or 'b' to go back: ").strip()
                    if choice.lower() == 'b':
                        break
                    choice_num = int(choice)
                    if 1 <= choice_num <= len(patients):
                        selected_patient_id = patients[choice_num - 1]
                        break
                    else:
                        print(f"Please enter a number between 1 and {len(patients)}")
                except ValueError:
                    print(f"Invalid input. Please enter a number between 1 and {len(patients)}")
            else:
                continue  # Go back to search if user pressed 'b'
            
            # Step 4: Get patient details
            print("\n" + "="*100)
            print(f"PATIENT DETAILS (ID: {selected_patient_id})")
            print("="*100)
            print(f"[Progress] Retrieving patient information from database...", flush=True)
            patient_info = await get_patient_summary(client, selected_patient_id)
            print(patient_info)
            
            # Step 5: Generate summary using Ollama
            print("\n" + "-"*100)
            print("STEP 3: GENERATE SUMMARY USING OLLAMA PHI LLM (WITH STREAMING TOKENS)")
            print("-"*100)
            print("\n⏳ Generating clinical summary using Phi model...")
            print("   📡 Will show tokens in real-time as they are generated (30-60 seconds)")
            print(f"[Progress] Sending patient data to Ollama...", flush=True)
            print(f"[Progress] LLM inference in progress... (WebSocket timeout: 120s)", flush=True)
            print(f"\n{'='*100}")
            print("LIVE STREAMING TOKENS - Watch as the summary is generated token-by-token:")
            print(f"{'='*100}\n")
            
            generated_summary = await generate_summary(client, selected_patient_id)
            
            print(f"\n{'='*100}")
            print("GENERATED SUMMARY")
            print("="*100)
            print(generated_summary)
            
            # Step 6: Save summary
            print("\n" + "-"*100)
            print("STEP 4: SAVE SUMMARY")
            print("-"*100)
            confirm = input("\nSave this summary to patient_summaries.csv? (y/n): ").strip().lower()
            
            if confirm == 'y':
                # Extract just the summary part from the generated text
                print(f"\n[Progress] Extracting summary text...", flush=True)
                summary_lines = generated_summary.split('\n')
                summary_start = None
                
                # Find where the actual summary starts
                for i, line in enumerate(summary_lines):
                    if 'Generated Summary for' in line:
                        summary_start = i + 2  # Skip title and blank line
                        break
                
                # Extract summary text
                if summary_start and summary_start < len(summary_lines):
                    summary_text = '\n'.join(summary_lines[summary_start:]).strip()
                else:
                    summary_text = generated_summary.strip()
                
                # Clean up multiple blank lines
                summary_text = '\n'.join([ln.strip() for ln in summary_text.split('\n') if ln.strip()])
                
                print(f"\n📝 Extracted summary ({len(summary_text)} characters):")
                if summary_text:
                    print(f"   {summary_text[:80]}...")
                    print(f"   [DEBUG] Full extracted text length: {len(summary_text)}")
                    print(f"   [DEBUG] First 100 chars: {summary_text[:100]}")
                else:
                    print(f"   ⚠️  WARNING: Extracted summary is EMPTY!")
                    print(f"   [DEBUG] This means the extraction logic may have failed")
                
                if not summary_text or len(summary_text) == 0:
                    print(f"\n❌ ERROR: Cannot save empty summary. Extraction failed.")
                    print(f"   Generated text was:")
                    print(f"   {generated_summary[:200]}")
                    continue
                
                print(f"[Progress] Sending summary to server for persistence...", flush=True)
                update_result = await update_summary(client, selected_patient_id, summary_text)
                
                print("\n" + "="*100)
                print(update_result)
                print("="*100 + "\n")
                print("✓ Summary saved successfully!")
                print(f"[Info] Updated patient_summaries.csv with {len(summary_text)} character summary", flush=True)
            else:
                print("\n✗ Summary not saved.")
            
            # Ask if user wants to continue
            print("\n" + "-"*100)
            continue_choice = input("Would you like to search for another patient? (y/n): ").strip().lower()
            if continue_choice != 'y':
                print("\n✓ Thank you for using the Patient Summary System. Goodbye!")
                break
    
    finally:
        await client.close()


if __name__ == "__main__":
    asyncio.run(main())
